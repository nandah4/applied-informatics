...
BEGIN
    ...
    INSERT INTO trx_aktivitas_lab (judul_aktivitas, deskripsi, foto_aktivitas, tanggal_kegiatan)
    VALUES (TRIM(p_judul_aktivitas), p_deskripsi, p_foto_aktivitas, p_tanggal_kegiatan);

    RAISE NOTICE 'Aktivitas "%" berhasil ditambahkan', p_judul_aktivitas;
END;
$$;

COMMENT ON PROCEDURE sp_insert_aktivitas IS
'Procedure untuk insert aktivitas baru. Validasi judul unik (case-insensitive).';
...
COMMENT ON PROCEDURE sp_update_aktivitas IS
'Procedure untuk update aktivitas. Validasi judul unik (case-insensitive) dan ID.';
